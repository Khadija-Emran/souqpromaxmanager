...
class StoreSettingModel extends ChangeNotifier {
  ...
  Future<Map<dynamic, dynamic>> prepareData() async {
    ...
    if (_storeLogo is XFile) {
      data['logo'] = await ImageTools.compressImage(_storeLogo);
    }
    if (banner['mobileBanner'] is XFile) {
      data['mobile_banner'] =
    ...
    if (banner['banner'] is XFile && banner['bannerType'] == 'static') {
    ...
    if (banner['storeListBanner'] is XFile &&
  ...
  Future<void> takeGalleryImages() async {
    try {
      ...
      final list = await ImageTools.pickMultiImage(
        limit: count as int,
      );
      if (list.isNotEmpty) {
